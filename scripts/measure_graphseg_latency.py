#!/usr/bin/env python3
"""GraphSeg-window per-turn latency split (Pre. vs Seg., idle CPU).

Pre. = tokenize + POS + GloVe vector lookup/stack (= `_preprocess_sec` + `_neural_sec`
both are non-decision feature extraction). Seg. = IC-weighted cosine + Hungarian +
graph + clique merge + emit (push() 나머지).
"""
...
from hi_em.baselines._seg_utils import (  # noqa: E402
    latency_stats, parse_defdts_dialogue)
from hi_em.baselines.graphseg_window import GraphSegWindowD  # noqa: E402

DEFDTS = REPO / "benchmarks" / "Def-DTS" / "data" / "DTS_session_datasets"
GLOVE = REPO / "benchmarks" / "glove" / "glove.6B.300d.txt"
...
def main():
    ...
    ap.add_argument("--name", default="2026-05-24_graphseg_latency_split")
    args = ap.parse_args()

    out_dir = REPO / "outputs" / "experiments" / args.name
    out_dir.mkdir(parents=True, exist_ok=True)

    print("[setup] loading GloVe + IC table ...", flush=True)
    t0 = time.perf_counter()
    proto = GraphSegWindowD(glove_path=str(GLOVE))
    proto._ensure_resources()
    cold_s = time.perf_counter() - t0
    print(f"[setup] loaded in {cold_s:.2f}s "
          f"(glove={len(proto._glove)} words)", flush=True)
    ...
    for ds in ("tiage", "dialseg711", "superseg"):
        ...
        for did, utts, _ in dialogs:
            seg = GraphSegWindowD(glove_path=str(GLOVE))
            seg._glove = proto._glove
            seg._glove_dim = proto._glove_dim
            seg._ic_table = proto._ic_table
            ...
            for i, u in enumerate(utts):
                ...
                d_pp = (seg._preprocess_sec - pp_before) * 1000.0
                d_nn = (seg._neural_sec - nn_before) * 1000.0
                t_pre = d_pp + d_nn   # 둘 다 feature extraction
                t_seg = t_total - t_pre
                if i >= 1:
                    pre_ms.append(t_pre)
                    seg_ms.append(t_seg)
    ...
    (out_dir / "latency.json").write_text(json.dumps(dict(
        method="GraphSeg-window", cold_start_s=cold_s, per_bench=results,
        all_bench=dict(pre=pre_all, seg=seg_all, total=tot_all),
        note="Pre. = tokenize + POS + GloVe lookup/stack. Seg. = IC×cosine + "
             "Hungarian + clique merge + emit."), indent=2))

    L = ["# GraphSeg-window per-turn latency split", "",
         "## 정의",
         "- **Pre.** = tokenize + POS + GloVe lookup/stack (_preprocess_sec + _neural_sec)",
         "- **Seg.** = IC-weighted cosine + Hungarian + graph + clique + emit",
         f"- cold-start (GloVe load) {cold_s:.2f}s 분리.",
         "", "## 결과",
