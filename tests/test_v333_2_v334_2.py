...
def test_v333_2_runs_and_returns_two_topics():
    seg = HiEMSegmenterV333_2(dim=16, alpha=10.0, lmda=10.0, cos_threshold=0.7)
...
def test_v333_2_prototype_max_capped():
    seg = HiEMSegmenterV333_2(dim=16, f0_proto_max=2)
...
def test_v333_2_invalid_p_threshold():
    with pytest.raises(ValueError):
        HiEMSegmenterV333_2(dim=8, restart_p_threshold=0.0)
    with pytest.raises(ValueError):
        HiEMSegmenterV333_2(dim=8, restart_p_threshold=1.0)
...
def test_v334_2_runs_and_returns_two_topics():
    seg = HiEMSegmenterV334_2(dim=16, alpha=10.0, lmda=10.0, cos_threshold=0.7)
    out = [seg.assign(s) for s in _two_topic_seq()]
    ks = [k for k, _ in out]
    assert len(set(ks)) >= 2
