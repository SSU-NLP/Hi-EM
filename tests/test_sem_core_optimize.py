"""Smoke + behavior tests for HiEMSegmenterV3 (Bounded Cosine MAP).
...
def test_segmenter_v3_first_assignment_is_topic_zero():
    ...
    seg = HiEMSegmenterV3(dim=8, tau=10.0)
...
def test_segmenter_v3_segments_orthogonal_clusters_low_sticky():
    ...
    seg = HiEMSegmenterV3(dim=768, tau=100.0, alpha=10.0, lmda=0.1)
...
def test_segmenter_v3_predict_topic_does_not_mutate():
    ...
    seg = HiEMSegmenterV3(dim=32, tau=20.0)
    for _ in range(5):
