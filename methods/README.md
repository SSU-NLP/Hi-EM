...
| `CSM/online/delay2.py` | **CSM-online-delay2** (streaming CoherenceNet + TextTiling-style depth, delay=2 right context). `src/hi_em/baselines/csm_online.py` 의 `CSMOnlineDelay2` wrap. lxing532 `CoherenceNet` 가중치 (ckpt=`outputs/runs/_misc/cpt_277000.pth`, backbone `bert-base-uncased`) 전제. 2026-05-23 신규. |
