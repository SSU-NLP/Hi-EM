...
## 현재 작업 범위 = Hi-OnTop 만 (최상위 규칙, 2026-06-10)

**지금 우리는 Hi-EM(분절기로 working memory 를 관리하던 메모리 파이프라인)은 일절 건드리지 않는다. 작업 대상은 오직 화제분절기 `Hi-OnTop` 하나다.** Hi-EM 의 STM/LTM/retrieval/promotion 등 메모리 메커니즘은 현재 scope 밖 — 수정·실험·설계 변경 모두 Hi-OnTop 분절 알고리즘 자체에 한정한다.

이 규칙의 직접 귀결:
- **codex 위임 프롬프트에서 "Hi-EM" 은 절대 언급하지 않는다.** codex 에는 Hi-OnTop 분절기의 *문제와 해결지점* 만 묻는다. Hi-EM 프로젝트명·내부 규칙(retrieval policy, decision-log 관행 등)·메모리 파이프라인 맥락을 프롬프트에 넣지 않는다. (SEM/SEM2 계승은 Hi-OnTop 의 학술 계보이므로 언급 OK — SEM 은 Hi-EM 이 아니다.)
- 문서·실험·커밋 메시지에서도 현재 변경이 Hi-OnTop 분절에 관한 것임을 기준으로 판단한다.
