---
title: "Hi-DoTS 알고리즘 워크스루"
subtitle: "처음 보는 사람용 · src/hi_em/hi_dots.py — HiDoTS · 2026-05-22"
---

> 목적: Hi-DoTS segmenter 가 **한 turn 을 처리할 때 실제로 무슨 함수를
> 어떤 순서로 호출하는지**, SEM/segmentation 을 처음 보는 사람도 따라올
> 수 있게 풀어 쓴 문서. 코드와 1:1 대조 검증 완료 (문서 끝 검증 노트).
>
> **이 문서엔 안 도는 코드가 없다.** Hi-DoTS 는 Hi-EM v4.1.x 라인을
...
Hi-DoTS 의 알고리즘 전체는 이 δ 를 두 시간 척도에서 계산해 학습된 임계값
`δ*` 와 비교하는 것이다.

---

## 1. Hi-DoTS 는 무엇인가 (한 줄)

**Hi-DoTS = Hi-EM v4.1.x 라인이 경험적으로 *환원되는* 정직한 최소
온라인 segmenter.**

Hi-EM v4.1.x (`HiEMSegmenterV411` / `V413`) 는 SEM2 스타일의 무거운
...
Hi-DoTS 는 그 환원형을 그대로 독립 클래스로 구현한 것이다. 폐기된 SEM2
기계는 `archive/legacy_sem_ablation/` 에 *실행 가능한 채로* 보존된다
(감사 계보 코드). Hi-DoTS 의 출력은 v4.1.3 default 와 **byte 단위로 동일**
...
함수명은 모두 `hi_dots.py` 의 실제 메서드명이다. 아래 각 함수 블록은
...
`hi_dots.py` [L134](../../src/hi_em/hi_dots.py#L134)
...
`hi_dots.py` [L145](../../src/hi_em/hi_dots.py#L145)
...
`hi_dots.py` [L161](../../src/hi_em/hi_dots.py#L161)
...
- **의미** — δ_prev(예민, 진짜 경계 안 놓침)와 δ_ctx(둔감, 잡담에 안
  속음)의 가중 평균. **이 합성이 Hi-DoTS 의 핵심** — 단일 척도가 아니라
...
`hi_dots.py` [L190–197](../../src/hi_em/hi_dots.py#L190) — `assign` 이
...
**이 한 줄 부등식이 Hi-DoTS 경계 결정의 전부다.** (왜 prior·argmax·
...
`hi_dots.py` [L188, 199–216](../../src/hi_em/hi_dots.py#L188) — 같은
`assign` 호출의 마무리.
...
히스토그램, [hi_dots.py:233](../../src/hi_em/hi_dots.py#L233)):
...
`outputs/experiments/2026-05-21_v413_demo` — Hi-DoTS 출력이 v4.1.3 와
동일하므로 그대로 유효):
...
된다. Hi-EM v4.1.x (`HiEMSegmenterV411`/`V413`) 는 SEM2 기계를 다 갖고
...
`ctx_window=3` 이었으나 보고 config 와 어긋났던 것으로, Hi-DoTS 는 main
...
동일 config(m=2)에서 Hi-DoTS 출력이 v4.1.3 와 **byte-identical** 임이
검증됐다 — 세 벤치 38,242 turn 중 boundary 예측이 다른 turn **0개**,
Score 까지 자리수 동일 (`scripts/verify_hidots_parity.py`,
`outputs/experiments/2026-05-22_hidots_parity/REPORT.md`). 따라서 수치는
v4.1.3 와 같다:
...
- Hi-DoTS 는 wording 은 비슷하나 topic 이 바뀌는 경우(δ≈낮음인데 GT 경계)
