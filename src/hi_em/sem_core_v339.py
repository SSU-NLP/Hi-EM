...
class HiEMSegmenterV339:
    ...
    * **σδ²** is calibrated online from the observed δ_prev distribution
    ...
    def __init__(
        ...
        use_rnn: bool = True,  # False → skip RNN compute/train entirely
        # (optional off, NOT removal). RNN also auto-skipped when η==1
        # (its output is ×0 anyway) — pure speedup, identical result.
    ...
    def _delta_eff(self, k: int, s: np.ndarray) -> float:
        """δ_eff² = η·δ_prev² + (1-η)·δ_model². RNN (δ_model) skipped
        when not _uses_rnn (η=1 or use_rnn=False) — identical result,
        no GRU forward. Falls back before s_{t-1} exists (first turn)."""
        d_prev = self._delta_prev(s)
        if not self._uses_rnn():
            return d_prev if d_prev is not None else 0.0
        d_model = float(self._pe(k, s))
        if d_prev is None:
            return d_model
        eta = self.eta_prev
        return math.sqrt(
            eta * d_prev * d_prev + (1.0 - eta) * d_model * d_model
        )
    ...
    def assign(self, s: np.ndarray) -> tuple[int, bool]:
        ...
        self.prev_k = k
        self._prev_s = np.array(s, dtype=np.float64, copy=True)  # s_{t-1}
