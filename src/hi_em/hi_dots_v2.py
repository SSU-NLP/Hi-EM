"""Hi-DoTS-v2 — Hi-DoTS + TextTiling-style lexical-overlap correction.

Hi-DoTS-v2 keeps the entire :class:`~hi_em.hi_dots.HiDoTS` semantic
...
Motivation
----------
Hi-DoTS' known structural failure mode (see ``hi_dots.py`` docstring and
``context/methodology/hi-dots.md``): when wording stays similar but the
...
utterance text::
    ...
    delta_base(t) = a*delta_prev(t) + (1-a)*delta_ctx(t)   # = HiDoTS delta_eff
...
The lexical context window (``m_lex``, ``rho_lex``) mirrors Hi-DoTS'
...
structure of Hi-DoTS is preserved. See ``context/06-decision-log.md``
(2026-05-23) and ``context/methodology/hi-dots-v2.md``.
"""
...
from hi_em.baselines.texttiling_streaming import _DEFAULT_STOP, _tokenize
...
class HiDoTSV2(HiDoTS):
    """Hi-DoTS with a TextTiling-style lexical-overlap correction.

    All :class:`HiDoTS` arguments behave identically (they shape the
    unchanged ``delta_base`` term). Lexical arguments:

    Args:
        w_lex: lexical-residual weight. ``delta_eff_v2 = delta_base +
            w_lex * r_t * (lexdist - mu_lex)``. 0.0 -> reduces to Hi-DoTS.
    ...
    Per-turn readouts (in addition to the :class:`HiDoTS` ones):
    ...
    def assign(self, s: np.ndarray, text: str = "") -> tuple[int, bool]:  # type: ignore[override]
        ...
        if first_turn:
            # stream start — no s_{t-1}; keep HiDoTS turn-0 behaviour (delta=0)
            delta_eff = 0.0
        else:
            corr = self.w_lex * r_t * (lexdist - self.mu_lex)
            # clip to the cosine-distance range [0, 2]; at w_lex=0 this is a
            # no-op so delta_eff_v2 == HiDoTS delta_eff exactly (verified).
        ...
        # update streaming state — embedding window (HiDoTS) + lexical window
