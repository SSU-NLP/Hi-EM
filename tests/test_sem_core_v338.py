...
def test_pe_prior_controls_fresh_harshness() -> None:
    ...
    lo = HiEMSegmenterV338(dim=DIM, pe_prior=0.4)._L0
    hi = HiEMSegmenterV338(dim=DIM, pe_prior=1.0)._L0
    assert hi < lo < 0.0
...
def test_reproducible_with_seed() -> None:
    ...
    def run() -> list[tuple[int, bool]]:
        seg = HiEMSegmenterV338(dim=DIM, alpha=1.0, lmda=10.0, seed=3)
...
def test_invalid_pe_prior_rejected() -> None:
    with pytest.raises(ValueError):
        HiEMSegmenterV338(dim=DIM, pe_prior=-0.1)
    with pytest.raises(ValueError):
        HiEMSegmenterV338(dim=DIM, pe_prior=2.5)


def test_assign_api_contract() -> None:
    seg = HiEMSegmenterV338(dim=DIM)
