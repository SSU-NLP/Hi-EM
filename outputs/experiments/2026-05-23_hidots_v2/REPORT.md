...
- **목적**: Hi-DoTS 에 TextTiling 식 단어-빈도 겹침(lexical overlap) 보정항을 더한 Hi-DoTS-v2 가, 세 분절 벤치마크에서 Hi-DoTS-v1 대비 성능을 올리는지 확인. v1 의 알려진 실패 모드 — wording 은 비슷한데 topic 이 바뀌는 경계 — 를 lexical 신호로 보완하는 게 가설.
- **모델**: `src/hi_em/hi_dots_v2.py` `HiDoTSV2`. `δ_eff_v2 = clip_[0,2](δ_base + w_lex·r_t·(lexdist − μ_lex))` (clip 범위 = cosine-distance 자연 범위 [0,2]; w_lex=0 시 no-op → v1 과 byte-parity). `δ_base` = Hi-DoTS δ_eff (불변). `lexdist = 1 − cos_tf(L_{t-1}, x_t)`, L = 직전 m_lex turn 의 ρ_lex-감쇠 sublinear-TF 합, x_t = 현재 turn sublinear-TF. `r_t = min(1, min(n_ctx,n_t)/min_tokens)` 짧은-turn 게이트.
...
- **HP**: 임베딩 window m=2 ρ=0.7 a=0.5 (Hi-DoTS default). lexical m_lex=2 ρ_lex=0.7 min_tokens=3 (codex 2026-05-23 권고 default). 인코더 multi-qa-mpnet.
- **w_lex grid (작은 범위 calibration)**: [0.0, 0.05, 0.1, 0.15, 0.2, 0.3]. `w_lex=0.0` ⇒ Hi-DoTS-v2 가 Hi-DoTS-v1 로 정확히 환원 → baseline 행.
...
- Hi-DoTS-v1 (w_lex=0) mean-3 Score = **0.4927**. w_lex>0 전 구간이 mean-3
...
- **best-w_lex-on-test 표기(★)는 참고용**: w_lex 를 test Score 로 고른 것은
  ...
  **w_lex ≈ 0.15~0.20** 가 벤치별 test 튜닝 없이 쓸 수 있는 단일 default.
  ...
  위험.
- **다음 iteration 결정**: Hi-DoTS-v2 를 v1 *대체*로 승격하기엔 mean-3
...
- **superseg calib = validation**: superseg 는 cached train 임베딩이 없고 92k-turn train 로컬 인코딩이 비현실적. validation 은 held-out label-free split (Hi-DoTS HP sweep 과 동일 선택). p80/median 은 라벨 미사용 → leakage 아님이나 '학습 데이터' 와는 엄밀히 다름.
