#!/usr/bin/env python3
"""Hi-DoTS 실제 분절 시연 — 3개 벤치마크의 진짜 대화로.

각 벤치(TIAGE / Dialseg711 / SuperDialseg) test set 에서 예시 대화를 골라
Hi-DoTS(default config)를 turn 단위로 돌리고, 발화 텍스트 + δ_eff +
...
출력: outputs/experiments/2026-05-22_hidots_segmentation_demo/REPORT.md
"""
...
def segment(embs):
    seg = HiDoTS(dim=embs.shape[1], **HP)
    rows = []
    for s in embs:
...
def main() -> None:
    out_dir = REPO / "outputs" / "experiments" / "2026-05-22_hidots_segmentation_demo"
    out_dir.mkdir(parents=True, exist_ok=True)

    L = [
        "# Hi-DoTS 실제 분절 시연 — 3개 벤치마크 진짜 대화 + 신뢰도",
        "",
        "Hi-DoTS default config (`δ*=0.5594, m=2, ρ=0.7, a=0.5`) 를 각 벤치",
        "test set 의 실제 대화에 turn 단위로 돌린 결과. 발화는 데이터셋 원문",
        "(긴 것은 `…` 절단).",
        "",
        "## 신뢰도(비율)란",
        "",
        "Hi-DoTS 는 매 turn `graded = δ_eff / δ*` 를 낸다. 이걸 4 밴드로 나눈다:",
    ...
    for ds, title in DS:
        ...
        for ex_n, di in enumerate(idxs, 1):
            ...
            L += [
                ...
                "",
                f"GT 경계 {n_gt}개 · Hi-DoTS 예측 {n_pred}개 · "
