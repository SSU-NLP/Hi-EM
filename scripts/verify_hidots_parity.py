#!/usr/bin/env python3
"""Hi-DoTS vs v4.1.x (HiEMSegmenterV411) output-parity verification.
...
HiEMSegmenterV413 == HiEMSegmenterV411 for the segmentation decision
...
def main():
    ...
    lines = [
        "# Hi-DoTS ↔ v4.1.3 output-parity 검증",
        "",
        f"config: m={M} · ρ={RHO} · a={A} · δ*={DSTAR} (canonical TIAGE-cfg)",
        "v4.1.3 의 segmentation 결정 = HiEMSegmenterV411 (v4.1.3 는 출력 "
        ...
        "Score v411 | Score Hi-DoTS | F1 | Pk | WD | parity |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|:--:|",
    ]
    all_ok = True
    for ds in ["tiage", "dialseg711", "superseg"]:
        ...
        p_dots = [seg_hidots(e) for e in embs]
        ...
        print(f"[{ds:11s}] n_dial={len(dia):5d} n_turn={n_turn:6d}  "
              ...
              f"Score v411={s_v411['score']:.4f} Hi-DoTS={s_dots['score']:.4f}  "
    ...
    verdict = ("**모든 데이터셋에서 byte-identical** — Hi-DoTS 출력이 "
               "v4.1.3 와 완전히 동일."
               if all_ok else "**불일치 발견** — 아래 표 확인.")
    lines += ["", f"## 판정", "", verdict, "",
              "diff turns = v411 과 Hi-DoTS 의 boundary 예측이 다른 turn 수. "
              "0 이면 두 segmenter 의 turn-level 출력이 완전히 같다는 뜻."]
    out = REPO / "outputs" / "experiments" / "2026-05-22_hidots_parity"
