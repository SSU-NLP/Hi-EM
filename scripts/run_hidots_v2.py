#!/usr/bin/env python3
"""Hi-DoTS-v2 (lexical-overlap correction) — 3-benchmark evaluation.

Hi-DoTS-v2 (``src/hi_em/hi_dots_v2.py``) adds a TextTiling-style
word-frequency overlap correction to Hi-DoTS' semantic ``delta_eff``:
...
- superseg   : calib = superseg/validation,   test = superseg/test
  ...
  label-free split — the same set the Hi-DoTS HP sweep used. p80 / median
...
``w_lex = 0.0`` reduces Hi-DoTS-v2 exactly to Hi-DoTS-v1 and serves as
the baseline row. Other HPs fixed at Hi-DoTS default
...
from hi_em.hi_dots_v2 import HiDoTSV2  # noqa: E402
...
# Hi-DoTS embedding-window HP (canonical default)
...
# --------------------------------------------------------------------------
# Hi-DoTS-v2 run helpers
# --------------------------------------------------------------------------

def _run_v2(emb, utts, w_lex: float, mu_lex: float, delta_star: float):
    """Run Hi-DoTSV2 over one dialogue; return the per-turn history list."""
    seg = HiDoTSV2(dim=emb.shape[1], delta_star=delta_star, w_lex=w_lex,
...
def main() -> None:
    ...
    ap.add_argument("--name", default="2026-05-23_hidots_v2")
    args = ap.parse_args()
    ...
    for ds in ("tiage", "superseg", "dialseg711"):
        ...
        for w_lex in W_LEX_GRID:
            ...
            tag = "  (= Hi-DoTS-v1)" if w_lex == 0.0 else ""
...
def write_report(exp_dir: Path, results: dict, summary: dict) -> None:
    ...
    L += [
        ...
        f"a={EMB_HP['ctx_blend_a']} (Hi-DoTS default). lexical "
        ...
        "`w_lex=0.0` ⇒ Hi-DoTS-v2 가 Hi-DoTS-v1 로 정확히 환원 → baseline 행.",
    ...
    L += [
        "## 3. 해석",
        "",
        f"- Hi-DoTS-v1 (w_lex=0) mean-3 Score = **{v1_mean:.4f}**.",
    ...
    L += [
        ...
        "Hi-DoTS-v2 를 default 후보로 승격 검토. 불일치/회귀면 v1 유지, "
        ...
        "label-free split (Hi-DoTS HP sweep 과 동일 선택). p80/median 은 라벨 "
