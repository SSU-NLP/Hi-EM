# Hi-DoTS vs online 베이스라인 비교표 (Def-DTS 번들, 벤치별)
...
- Hi-DoTS HP: m=2, ρ=0.7, a=0.5, 인코더 minilm-int8 (sentence-transformers/all-MiniLM-L6-v2).
- best-Score δ* 탐색 범위: [0.35, 0.95] (사용자 지정).
...
δ* — p80=0.7756 · best-Score=0.7466 (train split (300/300 dial))
...
| **Ours (p80)** | 0.4222 | 0.4729 | 0.3508 | **0.4516** | 0.0536 |
| Ours (best-Score)‡ref | 0.4337 | 0.5137 | 0.3756 | 0.4509 | 0.0366 |

## dialseg711  (test 711 dialog)

δ* — p80=0.7830 · best-Score=0.7873 (‡ train split 부재 → test 보정 (in-domain upper-bound))
...
| CSM-style | _대기_ | _대기_ | _대기_ | _대기_ | _대기_ |
| **Ours (p80)** | 0.3221 | 0.3622 | 0.5362 | **0.5970** | 0.0343 |
| Ours (best-Score)‡ref | 0.3211 | 0.3587 | 0.5336 | 0.5968 | 0.0314 |

## superseg  (test 1322 dialog)

δ* — p80=0.7951 · best-Score=0.5941 (train split (600/6948 dial subsample))
...
| CSM-style | _대기_ | _대기_ | _대기_ | _대기_ | _대기_ |
| **Ours (p80)** | 0.5425 | 0.5632 | 0.2694 | **0.3583** | 0.0725 |
| Ours (best-Score)‡ref | 0.4809 | 0.6971 | 0.4386 | 0.4248 | 0.0645 |

## 한계 / 검증 미해결
- 데이터 = Def-DTS 번들. Hi-DoTS 의 superdialseg_data 보고 수치(0.4675 등)와는 dialseg711/superseg turn 수가 달라 직접 일치 안 함.
