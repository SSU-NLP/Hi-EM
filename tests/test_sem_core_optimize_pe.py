"""Behavior tests for HiEMSegmenterV32 (Bounded Cosine MAP + sub-linear sCRP count)."""
...
def test_v32_rejects_invalid_beta():
    ...
    with pytest.raises(ValueError):
        HiEMSegmenterV32(dim=8, beta=0.0)
    with pytest.raises(ValueError):
        HiEMSegmenterV32(dim=8, beta=1.5)


def test_v32_first_assignment_creates_topic_zero():
    rng = np.random.default_rng(0)
    seg = HiEMSegmenterV32(dim=16, beta=0.5)
...
def test_v32_beta_one_matches_v31_segmentation():
    ...
    seg_v31 = HiEMSegmenterV3(dim=16, alpha=10.0, lmda=10.0, tau=50.0, cos_threshold=0.3)
    seg_v32 = HiEMSegmenterV32(
