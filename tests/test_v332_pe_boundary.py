...
def test_v332_disabled_pe_threshold_matches_v331_segmentation():
    ...
    torch.manual_seed(0)
    seg_a = HiEMSegmenterV331(dim=8, alpha=1.0, lmda=10.0, tau=50.0,
                              cos_threshold=0.5, beta=0.5,
                              rnn_hidden_dim=4, rnn_train_steps=1,
                              rnn_min_history=2)
    torch.manual_seed(0)
    seg_b = HiEMSegmenterV332(dim=8, alpha=1.0, lmda=10.0, tau=50.0,
                              cos_threshold=0.5, beta=0.5,
...
def test_v332_aggressive_pe_threshold_creates_more_topics():
    ...
    torch.manual_seed(0)
    seg_default = HiEMSegmenterV331(dim=8, alpha=1.0, lmda=10.0, tau=50.0,
                                    cos_threshold=0.5, beta=0.5,
                                    rnn_hidden_dim=4, rnn_train_steps=1,
                                    rnn_min_history=2)
    torch.manual_seed(0)
    seg_aggressive = HiEMSegmenterV332(dim=8, alpha=1.0, lmda=10.0, tau=50.0,
                                        cos_threshold=0.5, beta=0.5,
...
def test_v332_predict_topic_does_not_mutate():
    ...
    seg = HiEMSegmenterV332(dim=16, pe_threshold=0.5,
                             rnn_hidden_dim=4, rnn_train_steps=1)
    for _ in range(10):
...
def test_v332_invalid_pe_threshold_rejected():
    ...
    with pytest.raises(ValueError):
        HiEMSegmenterV332(dim=8, pe_threshold=-0.1)
    with pytest.raises(ValueError):
        HiEMSegmenterV332(dim=8, pe_threshold=1.5)
...
def test_v332_pe_threshold_zero_forces_new_topic_every_turn():
    ...
    seg = HiEMSegmenterV332(dim=8, pe_threshold=0.0,
                             rnn_hidden_dim=4, rnn_train_steps=1,
                             k_max=64)
