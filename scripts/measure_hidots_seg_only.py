"""Clean per-variant Seg-only latency (HiDoTS.assign timing).

For each (encoder × variant), measure HiDoTS.assign() timing using
...
Output: outputs/experiments/2026-05-24_hidots_seg_only_latency/
...
OUT_DIR = REPO / "outputs" / "experiments" / "2026-05-24_hidots_seg_only_latency"
...
def time_one_bench(embs, dstar, n_warmup=200, n_seeds=3, sample_size=1500):
    """Time HiDoTS.assign() on samples from one bench's embeddings."""
    ...
    for seed in range(n_seeds):
        ...
        # warmup
        seg = HiDoTS(dim=sample[0].shape[0], delta_star=dstar,
...
def main():
    ...
    L = ["# Hi-OnTop per-variant Seg latency (HiDoTS.assign timing, per-bench δ*)",
         "", "## 정의",
         "- 각 (variant × bench) 의 *실제 δ\\** 로 `HiDoTS.assign()` 만 perf_counter.",
