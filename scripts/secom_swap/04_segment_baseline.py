...
as ``scripts/run_plainprompt_online.py`` (the canonical Hi-EM-on-Crts entry):

    OPENAI_API_KEY  — Crts chat key (e.g. lab-... with chat scope)
    OPENAI_BASE_URL — https://api.ssunlp.co.kr/v1

Default segment model = ``qwen/qwen3.5-9b`` (matches Hi-EM's prior usage).
...
def main() -> None:
    ...
    # Hi-EM canonical pattern (run_plainprompt_online.py): OPENAI_API_KEY +
