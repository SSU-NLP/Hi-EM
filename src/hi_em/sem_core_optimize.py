...
Hi-EM-full-v3.1 replaces the v2 Gaussian likelihood with a cosine score::

    e_hat_n = argmax_k [ log prior(k | prev) + tau * cos(s_n, mu_k) ]

This is the centroid-cosine variant. v3.2 (in
:mod:`hi_em.sem_core_optimize_pe`) replaces the centroid with an
...
from :mod:`hi_em.scrp`. Subsequent v3 phases (prior decay /
...
from hi_em.topic_optimize import TopicV3


class HiEMSegmenterV3:
