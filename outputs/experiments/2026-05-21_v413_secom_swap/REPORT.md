...
`outputs/experiments/2026-05-21_v413_secom_swap/` · 2026-05-21~22 · ours = Hi-DoTS 재실행 완료

## 한 줄

SeCom (Pan et al., ICLR 2025) 의 LLM 기반 segmentation backend (`gpt-4o-mini`) 를
Hi-EM Hi-DoTS (online, O(m)/turn) 으로 drop-in 교체. **downstream QA 품질이 baseline LLM
...
| **(ours) Ours: Hi-DoTS** | MPNet | **Hi-EM Hi-DoTS (online, O(m)/turn)** | `openai/gpt-4o-mini` |
...
**Hi-DoTS segmentation 파라미터** (2026-05-22 재실행):
...
- δ\* = **0.5983** — mpnet δ_eff p80, **ctx_window=2 기준 재보정**
  (`delta_star_calibration_hidots_m2.json`). bge TIAGE δ* (≈0.5594) 는 인코더 불일치로 부적합.
  직전 run 은 δ_prev p80 = 0.6194 (m=3) 사용.
- Hi-DoTS HP: m(ctx_window)=2, ρ(ctx_decay)=0.7, a(ctx_blend_a)=0.5. v4.1.x 의 α/λ/β/pe/f0
  등은 Hi-DoTS reduced form 에 존재하지 않음 (dead-code audit 으로 제거).
...
| **3** | **Hi-Seg (Ours, Hi-DoTS)** | **77.40** | 19.63 | 39.47 | 23.21 | 32.72 | **88.91** | 4.27 | 1,124 | **0.07** ⭐ |
...
- Hi-Seg row = **Hi-DoTS** (m=2, δ*=0.5983, Crts mpnet) — 2026-05-22 재실행. 직전 v4.1.3 (m=3, δ*=0.6194: GPT4Score 74.72) 대비 전 지표 향상. 변경 이력 참조.
...
| Metric | baseline | Hi-Seg (Hi-DoTS) | Δ |
|---|---:|---:|---:|
...
직전 v4.1.3 run 의 -3.40pp (GPT4Score) 대비 크게 개선. Segment algorithmic latency 는 Hi-DoTS
...
| **ours (Hi-DoTS, Crts encode)** | 188 | 3.83 | 78.7 | 0.053 | **0.074** | **109** |
| ours (v4.1.3, 로컬 CPU encode) | 167 | 4.31 | 643 | 3.74 | 5.20 | 903 |

- **algorithmic latency (segmenter assign 만)**: Hi-DoTS **0.074 ms/turn** vs baseline **646 ms/turn**.
  Hi-DoTS reduced form 은 v4.1.3 (5.20 ms) 가 돌리던 dead SEM2 machinery 를 제거 → assign 이 순수
  causal-window cosine + threshold 만 → 70× 더 빠름. 출력은 v4.1.3 와 byte-identical (parity 검증).
- **end-to-end (text → vector → segment)**: Hi-DoTS = **109 ms/turn** — encode 를 Crts API 로 offload
...
> `segment_compare.json` — Hi-DoTS 재실행 (m=2, δ*=0.5983, 188 seg) 에 대해서는 미재계산.

## 해석 (Hi-DoTS run, 2026-05-22)

1. **Algorithmic claim 입증**: Hi-DoTS `assign()` 0.074 ms/turn vs LLM 646 ms/turn. reduced
...
3. **Trade-off 정량화**: Hi-DoTS 는 baseline 보다 굵은 segments (3.83 vs 2.26 ex/seg) →
...
- **drop-in 교체 가능 ✅**: SeCom 의 LLM segmentation 백엔드를 Hi-DoTS 로 교체 시 downstream QA
  의 main metric (GPT4Score 99.1% / BERTScore-F1 99.7% 유지), segmenter assign 은 ~8700× 빠름
  (encode 포함 end-to-end 도 baseline LLM 콜보다 빠름).
- **Paper contribution 정량 입증**: "Hi-EM Hi-DoTS 의 graded boundary score segmenter 는 LLM
...
- **assign-only 0.074 ms 의 비약**: v4.1.3 (5.20 ms) 대비 70× — Hi-DoTS reduced form 이
  dead SEM2 machinery 를 안 돌리기 때문. 출력 parity 는 검증됐으나 latency 차이의 단일
  측정값이라 repeated-run 분산 미측정.
- **Boundary placement / segment_compare 는 v4.1.3 run 기준** — Hi-DoTS (188 seg) 재계산 안 함.
...
- `scripts/secom_swap/03_segment_v413.py` — Hi-DoTS segmentation runner (`--encoder_backend api`)
...
- `delta_star_calibration_hidots_m2.json` — mpnet δ_eff p80 = 0.5983 (Hi-DoTS run, m=2)
- `latency_ours.json` / `latency_baseline.json` — v4.1.3 / baseline timing
- `latency_ours_hidots.json` — Hi-DoTS run timing
- `metrics_ours.json` (v4.1.3) / `metrics_ours_hidots.json` (Hi-DoTS) / `metrics_baseline.json` — downstream eval
...
- **2026-05-22 (ours = Hi-DoTS 재실행)**: ours row 를 v4.1.3 → Hi-DoTS 로 재실행. ① segment 인코더를 Crts `/v1/embeddings` API 로 전환 (`03_segment_v413.py --encoder_backend api`, `secom_adapter._encode` 가 두 인코더 contract 지원). ② ctx_window default 3→2 변경에 맞춰 δ* 를 δ_eff p80 (m=2) = **0.5983** 으로 재보정 (`02_calibrate_delta_star.py --mode delta_eff`). ③ segment(188 seg) → compress → retrieve → chat → eval. 결과: GPT4Score 74.72→**77.40**, BERTScore-F1 88.69→**88.91**, baseline 대비 gap -3.40→**-0.72pp**. v4.1.3 산출물 (`metrics_ours.json` 등) 은 보존. 8-method 표·Δ표·Latency표·해석·판정 모두 Hi-DoTS 기준으로 갱신
