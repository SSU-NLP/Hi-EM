#!/usr/bin/env python3
"""Hi-DoTS per-turn latency (realtime, no encoder cache, single-utt forward).
...
def main() -> None:
    ...
    ap.add_argument("--name", default="2026-05-23_hidots_latency_realtime")
    ...
    for ds in ("tiage", "dialseg711", "superseg"):
        ...
        for did, utts, _ in dialogs:
            seg = HiDoTS(dim=768, delta_star=0.5594,
    ...
    payload = dict(
        ...
        note=("per-turn = encode(single utt, no cache) + assign(HiDoTS). "
    ...
    L += [
        ...
        "single-utt forward).",
        f"- **Seg.** 셀 = {seg_all['mean']:.3f} ms (cross-bench mean, HiDoTS.assign).",
