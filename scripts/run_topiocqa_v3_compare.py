...
def run_v3_1(convs, embeddings, alpha, lmda, tau, cos_threshold=0.5):
    ...
    max_share_per_conv = []
    for conv, emb in zip(convs, embeddings):
        seg = HiEMSegmenterV3(dim=emb.shape[1], alpha=alpha, lmda=lmda,
...
def main() -> None:
    ...
    lines = [
        "# TopiOCQA — Hi-EM 변형 모두 비교 (v2 / v3.1 / v3.2)",
        "",
        ...
        "|---|---|---|---|---|---|",
        f"| v2 hi-em-full-v1 (Gaussian) | {p2:.3f} | {r2:.3f} | {f1_2:.3f} | "
        f"{np.mean(ntopics2):.1f} | {np.mean(mshare2):.3f} |",
        f"| v3.1 hi-em-full-v3.1.1 (Bounded Cosine MAP, τ={tau31}, thr={thr31}) | "
        f"{p31:.3f} | {r31:.3f} | {f1_31:.3f} | "
        f"{np.mean(ntopics31):.1f} | {np.mean(mshare31):.3f} |",
        f"| v3.2 hi-em-full-v3.2.1 (Cosine PE, τ={tau32}, thr={thr32}, ρ={rho32}) | "
