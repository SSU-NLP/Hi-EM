...
Hi-EM Hi-OnTop (online, O(m)/turn) 으로 drop-in 교체. **downstream QA 품질이 baseline LLM
...
| **(ours) Ours: Hi-OnTop** | MPNet | **Hi-EM Hi-OnTop (online, O(m)/turn)** | `openai/gpt-4o-mini` |
...
- **Paper contribution 정량 입증**: "Hi-EM Hi-OnTop 의 graded boundary score segmenter 는 LLM
