...
SWEEP = Path("/home/namchailin/Hi-EM/outputs/experiments/2026-05-12_v3342_sweep_full")
INITIAL_THRESHOLD = 0.266
POLL_SEC = 3
RUN_RE = re.compile(r"^(v3342_s\d+_c\d+)_r(\d+)$")
...
def get_acc(run: str) -> float | None:
    sum_path = SWEEP / run / "results" / "experiments" / f"2026-05-12_v3342_sweep_full_{run}" / "summary.json"
...
def sweep_alive() -> bool:
    try:
        subprocess.check_call(
            ["pgrep", "-f", "scripts/experiment.py.*2026-05-12_v3342_sweep_full"],
