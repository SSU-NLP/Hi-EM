...
New MAJOR-minor line on top of v4.1.1 (``sem_core_v411.HiEMSegmenterV411``,
...
hardcodes for CM eval, so swapping it in here aligns Hi-EM's scene-vector
...
- Set ``HIEM_EMBEDDING_MODEL=aws-ai/dse-bert-base`` in ``.env``
  (``HIEM_EMBEDDING_BACKEND=api`` if served via Crts; ``=local`` if
...
from hi_em.sem_core_v411 import HiEMSegmenterV411

EXPECTED_ENCODER = "aws-ai/dse-bert-base"


class HiEMSegmenterV422(HiEMSegmenterV411):
    ...
    orchestrator via ``HIEM_EMBEDDING_MODEL``; this class does not
    instantiate one.
