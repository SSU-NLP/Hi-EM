# Hi-DoTS-v2 — Hi-DoTS + lexical-overlap correction

`src/hi_em/hi_dots_v2.py` · `class HiDoTSV2(HiDoTS)` · 2026-05-23 신설

## 한 줄

[[hi-dots]] (`HiDoTS`) 의 의미거리 boundary 메커니즘은 **그대로 두고**,
...
[[hi-dots]] 한계 §: "wording 은 비슷한데 topic 이 바뀌는 경우(δ≈낮은데
...
```
δ_base(t)  = a·δ_prev(t) + (1−a)·δ_ctx(t)        # = HiDoTS δ_eff (불변)
...
- `clip_[0,2]` — cosine-distance 자연 범위. **w_lex=0 이면 보정항이 0
  이고 clip 도 no-op → δ_eff_v2 가 [[hi-dots]] δ_eff 와 정확히 일치**
  (parity 검증됨, smoke test).
- lexical causal window `(m_lex, ρ_lex)` 는 [[hi-dots]] 의 임베딩 causal
...
- 나머지 (`_prev_s`, `_recent`, `_topic_id`, `last_*`) 는 [[hi-dots]] 상속.

## HP

HiDoTS 임베딩 HP 4개 (`delta_star`, `ctx_window`, `ctx_decay`,
...
`scripts/run_hidots_v2.py` 가 세 벤치 calib split (tiage/train,
...
2. **충돌?** — 제한적. lexical 항은 sCRP prior 도 local-MAP inference 도
   아니므로 *SEM 원형 충실 재현* 목표와는 이질적. 그러나 Hi-DoTS-v2 의
   목표는 SEM 원형 재현이 아니라 SEM-inspired online dialogue
   segmentation — 이 관점에선 충돌 없음. lexical overlap 은 별도
   memory/retrieval 규칙이 아니라 현재 관측 `s_t` 주변 local boundary
   evidence 를 보강하는 domain-specific *observation correction*.
...
CLAUDE.md "Retrieval 은 importance score 만" 규칙과는 무관 (여기는
retrieval 이 아니라 segmentation). 핵심 제약은 SEM 계승 §.

## 결과 (3-벤치, official SuperDialseg Score)

`outputs/experiments/2026-05-23_hidots_v2/REPORT.md` (2026-05-23).
...
→ 현재 main 모델은 [[hi-dots]] (`HiDoTS`) 유지. `HiDoTSV2` 는 검증
대기 변형.
...
- **2026-05-23 신설** — [[hi-dots]] 에 lexical-overlap 보정 추가한
  `HiDoTSV2` 도입. 설계 codex:rescue 위임. w_lex=0 parity 검증.
