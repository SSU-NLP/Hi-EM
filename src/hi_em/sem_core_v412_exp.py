...
no learned model. A literal d-dim Gaussian port would clash with Hi-EM's
...
class HiEMSegmenterV412Exp:
    """v4.1.2-EXPERIMENTAL — v4.1.1 + per-topic δ*_k (codex 2026-05-19, P2).

    **EXPERIMENTAL, NOT default.** v4.1.1(전역 단일 δ*) 은 그대로 두고,
    토픽별 적응 임계 δ*_k 를 *long-dialogue 전용 실험 트랙* 으로 얹은
    버전. 검증 통과(Dialseg711 Score↑ & SuperDialseg Score/WD 유지 &
    guard 안정) 시에만 v4.1.2 승격. (decision-log 2026-05-19.)

    설계 (codex 안전 수식, v3.3.9-pre 붕괴 교훈 반영):
      D_k = {δ_eff(t) : accepted **continuation** of topic k}
            (boundary/fresh/same-label restart turn 제외 — within-segment
            only. 전체 δ 분포 추정 금지 = v3.3.9-pre 붕괴 차단.)
      n_k = |D_k|
      r_k = Q0.80(D_k) + κ·MAD(D_k)                 # robust, 평균/분산 금지
      γ_k = 1[n_k≥N_min]·γ_max·(m+1)/(m+1+ν), m=n_k−N_min   # shrink-to-global
      δ*_k = clip( (1−γ_k)·δ*_0 + γ_k·r_k , 0.85·δ*_0 , 1.15·δ*_0 )

    n_k<N_min → γ_k=0 → δ*_k=δ*_0 (전역과 **정확히 동일** = SuperDialseg
    같은 짧은 대화 graceful fallback, 무해 가드). prior-corrected fresh
    baseline 은 prev-topic δ*_{k_prev} 로 계산 → repeat>fresh ⟺
    δ_eff<δ*_{k_prev} 동치 유지 (v3.3.9 prior-correction 의 topic별
    일반화; prior 제거 아님). σδ² 는 **전역 고정 유지**(per-topic 화
    금지 — v3.3.9-pre 교훈). f0/비-prev 재진입 path 는 전역 δ*_0 유지.

    ─── inherited v4.1.1 docstring ───
    ...
    * **σδ²** is calibrated online from the observed δ_prev distribution
    ...
    def __init__(
        ...
        # v4.1.2-EXP — per-topic δ*_k (codex 2026-05-19, safe gated)
        pt_enabled: bool = True,  # False → pure v4.1.1 (전역 δ* only)
        pt_n_min: int = 6,  # N_min: 샘플 < 이면 전역 δ*_0 (fallback)
        pt_kappa: float = 0.5,  # κ: r_k = Q0.80 + κ·MAD
        pt_nu: float = 12.0,  # ν: shrink 분모
        pt_gamma_max: float = 0.35,  # γ_max: 최대 per-topic 가중
        pt_clip_lo: float = 0.85,  # δ*_k clip 하한 ×δ*_0
        pt_clip_hi: float = 1.15,  # δ*_k clip 상한 ×δ*_0
        ...
        # v4.1.2-EXP per-topic δ*_k params + within-continuation samples.
        if pt_n_min < 2:
            raise ValueError(f"pt_n_min must be >= 2, got {pt_n_min}")
        if not 0.0 <= pt_gamma_max <= 1.0:
            raise ValueError(f"pt_gamma_max in [0,1], got {pt_gamma_max}")
        if not 0.0 < pt_clip_lo <= 1.0 <= pt_clip_hi:
            raise ValueError("expect pt_clip_lo<=1<=pt_clip_hi")
        self.pt_enabled = pt_enabled
        self.pt_n_min = pt_n_min
        self.pt_kappa = pt_kappa
        self.pt_nu = pt_nu
        self.pt_gamma_max = pt_gamma_max
        self.pt_clip_lo = pt_clip_lo
        self.pt_clip_hi = pt_clip_hi
        # D_k: accepted within-continuation δ_eff per topic (boundary /
        # fresh / same-label-restart turn 제외). 전체 δ 분포 금지.
        self._dstar_samples: list[list[float]] = []
    ...
    def _ensure_topic_slot(self, k: int) -> None:
        while len(self.topics) <= k:
            ...
            self._dstar_samples.append([])  # v4.1.2-EXP per-topic D_k
    ...
    # ---- v4.1.2-EXP: per-topic δ*_k (codex 2026-05-19 safe spec) ------

    def _delta_star_for(self, k: int | None) -> float:
        """Per-topic threshold δ*_k. n_k<N_min → exactly δ*_0 (전역
        fallback, SuperDialseg 무해 가드). 충분 표본 → shrink-to-global:
        δ*_k = clip((1-γ)·δ*_0 + γ·(Q0.80+κ·MAD), [lo,hi]·δ*_0).
        D_k = accepted within-continuation δ_eff only (mixture 오염 차단)."""
        d0 = self.delta_star
        if (not self.pt_enabled or k is None
                or k >= len(self._dstar_samples)):
            return d0
        D = self._dstar_samples[k]
        n = len(D)
        if n < self.pt_n_min:
            return d0  # γ=0 → 전역과 정확히 동일
        a = np.asarray(D, dtype=np.float64)
        q80 = float(np.quantile(a, 0.80))
        mad = float(np.median(np.abs(a - np.median(a))))
        r_k = q80 + self.pt_kappa * mad
        m = n - self.pt_n_min
        gamma = self.pt_gamma_max * (m + 1.0) / (m + 1.0 + self.pt_nu)
        dk = (1.0 - gamma) * d0 + gamma * r_k
        return float(np.clip(dk, self.pt_clip_lo * d0, self.pt_clip_hi * d0))

    def _record_dstar_sample(self, k: int, d_eff: float) -> None:
        """Append δ_eff to D_k — caller guarantees this is an *accepted
        within-continuation* turn (not boundary/fresh/restart)."""
        if 0 <= k < len(self._dstar_samples):
            buf = self._dstar_samples[k]
            buf.append(float(d_eff))
            if len(buf) > 256:  # cap (same spirit as pe_var_window)
                del buf[0]
    ...
    def _fresh_baseline_for_prev(
        ...
        """codex 2026-05-18/19: time-dependent, prior-corrected fresh
        baseline. v4.1.2-EXP uses the **prev-topic** δ*_{k_prev} (not the
        global δ*) so the repeat-vs-fresh MAP decision is EXACTLY
        ``δ_eff < δ*_{k_prev}`` every turn (prior cancels) — the topic-
        wise generalisation of v3.3.9's prior-correction. n_k<N_min →
        δ*_{k_prev}=δ*_0 → identical to v4.1.1 (graceful fallback)."""
        r_t = max(prior_prev, 1e-300) / max(prior_new, 1e-300)
        dstar_prev = self._delta_star_for(self.prev_k)
        return self._delta_loglik(dstar_prev) + math.log(r_t)
    ...
    def assign(self, s: np.ndarray) -> tuple[int, bool]:
        ...
        # v4.1.2-EXP: record δ_eff into D_k ONLY on accepted within-
        # continuation (boundary / fresh / same-label restart excluded —
        # mixture 오염 차단, v3.3.9-pre 붕괴 교훈). _prev_s/_recent 는
        # assign 끝에서 갱신 → 여기 _delta_eff 는 scoring 때 값과 동일.
        if self.pt_enabled and is_continuation:
            self._record_dstar_sample(k, float(self._delta_eff(k, s)))
