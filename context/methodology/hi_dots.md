# Hi-DoTS — Hi-EM Dialogue Topic Segmenter (reduced-form main model)

`src/hi_em/hi_dots.py` · 2026-05-22 신설 (v4.1.x dead-code audit 결과)

## 한 줄

Hi-EM v4.1.x 라인([[v4.1.1]] `HiEMSegmenterV411` / [[v4.1.3]]
`HiEMSegmenterV413`)을 dead-code 감사한 끝에, **출력에 실제로 영향을 주는
...
3. **decision-log**: 2026-05-22 (v4.1.x dead-code audit → Hi-DoTS
   reduced-form main model, commit `326b86b`).

Hi-DoTS 는 SEM 핵심 직관 — "다음 관측이 최근 사건 맥락으로 잘 예측되지
...
예측 diff 0, `scripts/verify_hidots_parity.py`,
`outputs/experiments/2026-05-22_hidots_parity/`):
...
- **frozen-pretrained f (δ_model 채널)** — v4.2.4/v4.3.x exp 라인에서
  ...
  single-η mean-best 기준 net-neutral~negative → 미승격. Hi-DoTS 는
...
- **2026-05-22 신설** — v4.1.x dead-code audit(commit `326b86b`)으로
  `src/hi_em/hi_dots.py` `HiDoTS` 도입. v4.1.3 와 output parity 검증
