...
* :class:`QueryEncoder` — local ``sentence-transformers`` (default
  ...
  similarity, matching Hi-EM's downstream sCRP / RAG math. Heavy deps
...
* :class:`APIEncoder` — OpenAI-compatible ``/v1/embeddings`` HTTP client
  ...
  with one short request at construction (``HIEM_EMBEDDING_DIM`` env
  overrides if probing is undesired).

:func:`make_encoder` is the entry-point factory: env-driven backend
selection (``HIEM_EMBEDDING_BACKEND=local|api``) so call sites can stay
agnostic.
...
Hi-EM preload/handle_turn can call ``encode()`` from multiple threads
safely.
"""
...
class _Encoder(Protocol):
    """Structural contract relied on by orchestrator / RAG."""
...
class APIEncoder:
    ...
    Args:
        ...
        api_key: Auth key. Falls back to ``HIEM_EMBEDDING_API_KEY`` then
            ``OPENAI_API_KEY``.
        base_url: API root. Falls back to ``HIEM_EMBEDDING_BASE_URL`` then
    ...
    def __init__(
        ...
        api_key = (
            api_key
            or os.environ.get("HIEM_EMBEDDING_API_KEY")
            or os.environ.get("OPENAI_API_KEY")
        )
        base_url = (
            base_url
            or os.environ.get("HIEM_EMBEDDING_BASE_URL")
            or os.environ.get("OPENAI_BASE_URL")
            or DEFAULT_API_BASE_URL
        )
        if not api_key:
            raise RuntimeError(
                "APIEncoder requires an API key — set HIEM_EMBEDDING_API_KEY "
...
def make_encoder(
    ...
    Resolution order:
        1. ``backend`` arg
        2. ``HIEM_EMBEDDING_BACKEND`` env var (``local`` | ``api``)
        3. default: ``local``
    ...
    Args:
        ...
        model: Override model name. Local default ``BGE_MODEL_NAME``;
            ...
            ``HIEM_EMBEDDING_MODEL``).

    Returns:
        An object exposing ``.dim`` and ``.encode()``.
    """
    backend = (backend or os.environ.get("HIEM_EMBEDDING_BACKEND") or "local").lower()
    if backend == "api":
        api_model = (
            model
            or os.environ.get("HIEM_EMBEDDING_MODEL")
            or DEFAULT_API_MODEL
        )
        dim_env = os.environ.get("HIEM_EMBEDDING_DIM")
        dim = int(dim_env) if dim_env else None
        dimensions_env = os.environ.get("HIEM_EMBEDDING_DIMENSIONS")
        dimensions = int(dimensions_env) if dimensions_env else None
        return APIEncoder(model=api_model, dim=dim, dimensions=dimensions)
    if backend == "local":
        local_model = model or os.environ.get("HIEM_EMBEDDING_MODEL") or BGE_MODEL_NAME
        return QueryEncoder(device=device, model_name=local_model)
    raise ValueError(
        f"Unknown HIEM_EMBEDDING_BACKEND={backend!r} (expected 'local' or 'api')"
    )
