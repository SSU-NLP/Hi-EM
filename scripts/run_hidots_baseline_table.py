#!/usr/bin/env python3
"""Hi-DoTS vs online 베이스라인 비교표 (Def-DTS 번들, 3 벤치 각각).

베이스라인(TextTiling/GreedySeg/GraphSeg)은 Def-DTS 번들 test set 에서
segeval Pk/WD + boundary-set F1 로 측정됨 → Hi-DoTS 도 **같은 데이터·같은
...
인코더 = multi-qa-mpnet (Hi-EM 기본, v4.1.x/Hi-DoTS 보고 수치와 동일 계열).
"""
...
M, RHO, A = 2, 0.7, 0.5  # causal-window HP (Hi-DoTS default)
...
def delta_eff_seq(emb):
    """HiDoTS 의 turn별 δ_eff (δ* 무관). history()[i]['delta_eff']."""
...
def eval_latency(dialogs, embs, delta_star):
    ...
    for (did, utts, gold), emb in zip(dialogs, embs):
        seg = HiDoTS(dim=emb.shape[1], delta_star=delta_star,
                     ctx_window=M, ctx_decay=RHO, ctx_blend_a=A)
        pred = []
...
def main():
    ...
    name = args.name or (
        "2026-05-22_hidots_baseline_table" if ENCODER == "mpnet"
        else f"2026-05-23_hidots_table_{ENCODER}")
    ...
    L = [
        ...
        "베이스라인과 1:1 공정비교 아님).",
        f"- Hi-DoTS HP: m={M}, ρ={RHO}, a={A}, 인코더 {ENCODER} ({ENCODERS[ENCODER]['model']}).",
    ...
    L += [
        "## 한계 / 검증 미해결",
        "- 데이터 = Def-DTS 번들. Hi-DoTS 의 superdialseg_data 보고 수치"
